...
### 恢复：15 秒语音试探只对 X 生效，抖音等平台直接全程转录（回归原始设计）

原始设计（README「X 推文提取——不转文案」+ 5 月版代码注释）：X 视频多为无意义
内容，先截前 10 秒试探，无连贯语音就跳过全程 whisper；抖音等其他平台直接全程
转录。后续改动把试探泛化到了所有平台，导致前奏是音乐/无人声的抖音视频会被
误跳过、丢文案。本次恢复：`_maybe_transcript`（15 秒试探）仅 X 走，
其余平台直接 `_run_whisper` 全程转录。
